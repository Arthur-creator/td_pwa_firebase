...
import { getProducts, getProduct, setCart, getCart } from './api/products';
...
(async (root) => {
  ...
  page('/cart', async () => {
    ...
    if (NETWORK_STATE) {
      ...
      products.id = 1
      cartProducts = await setRessource(products);
    } else {
        cartProducts = await getRessources();
    }
    AppCart.products = cartProducts.items;
    AppCart.active = true;
